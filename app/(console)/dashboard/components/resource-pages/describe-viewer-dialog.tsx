...
type DescribeViewerDialogProps = {
  open: boolean
...
export function DescribeViewerDialog({
  open,
  onOpenChange,
  title,
  subtitle = "展示资源详情输出。",
  ...
  const initTerminal = React.useCallback(() => {
    ...
    const initial = latestContentRef.current || "(无详情输出)"
  ...
  }, [content, error, loading, open, scheduleFit])
      ...
      >
        <div className="flex items-start justify-between border-b bg-muted/15">
          ...
          <div className="h-full flex items-center gap-3 me-20">
            {onDownload ? (
              ...
              >
                <IconDownload />
                <span className="sr-only">下载详情</span>
              </Button>
            ) : null}
            <Button
              type="button"
        ...
        <div className="min-h-0 flex-1 overflow-hidden p-6">
          <div className="relative h-full overflow-hidden rounded-lg border border-slate-700/60 bg-slate-950/95 shadow-inner">
            ...
            {loading ? (
              <div className="absolute inset-0 bg-black/60 p-4 text-sm text-zinc-300">详情加载中...</div>
            ) : null}
            {error ? (
