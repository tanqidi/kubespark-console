...
export function useCreateWorkloadDialogController(props: CreateWorkloadDialogProps) {
  ...
  const [schedulingPolicyEnabled, setSchedulingPolicyEnabled] = React.useState(false)
  ...
  const confirmYamlMode = React.useCallback(() => {
    try {
      const source = withLockedIdentity(parseWorkloadYamlText(kind, yamlText))
      ...
      try {
        ...
        const normalizedSnapshot: WorkloadDialogSnapshot = {
          ...
          pod: {
            ...
            schedulingPolicyEnabled: source.pod.schedulingPolicyEnabled === true,
            schedulingPolicy:
