...
- 资源列表统一能力：
  - 分页、筛选、可配置自动刷新（默认关闭；配置 `NEXT_PUBLIC_AUTO_REFRESH_SECONDS` 后启用）。
  - 行操作（查看 YAML、删除等）。
  - 批量删除。
- 通用删除确认弹窗与 YAML 查看弹窗。
...
- 新增资源详情（Describe）能力：
  - 已接入：Namespace、Node、Pod、Service、Workload、Job/CronJob
  - 暂不接入：ConfigMap、Secret、PV/PVC、StorageClass、Ingress。
...
| 容器组（Pod） | ✅ | ✅ | N/A | ✅ | ✅ | 以运维查看/删除为主 |
| 应用路由（Ingress） | ✅ | ✅ | ✅ | ✅ | ✅ | 已支持路由规则多条录入、YAML 互转、编辑回显 |
| 存储卷（PV/PVC） | ✅ | ✅ | N/A | ✅ | ✅ | 已支持 PV/PVC 创建（表单 + YAML） |
| 存储类（StorageClass） | ✅ | N/A | N/A | ✅ | ✅ | 当前无创建/编辑弹窗 |
