...
- Pipeline 与 Workspace 的归属关系通过 `workspaceRef` 统一建模。
...
1. Admission Webhook：校验 `workspaceRef`、`namespaceRef` 的目标对象必须存在。
...
1. CRD 采用复数资源名（Kubernetes 约定）：`workspaces`、`pipelines`。
